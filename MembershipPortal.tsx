                      ...
                      {visiblePlans.map((plan) => (
                        <div key={plan.id} className="portal-plan-grid grid gap-4 border-b border-white/5 pb-4 last:border-b-0 md:gap-6 md:pb-6 lg:grid-cols-[1.1fr_0.9fr]">
                          <div className="portal-plan-card flex h-full flex-col rounded-[20px] border border-white/10 bg-[#121212]/50 p-5 transition-all duration-300 hover:border-gold/30 md:p-8">
                            ...
                            {/* Enquiry on WhatsApp — wired to existing handlePlanWhatsAppClick */}
                            <button
                              onClick={() => handlePlanWhatsAppClick(plan)}
                              className="portal-plan-button w-full rounded-full border border-[#25D366]/40 bg-[#25D366]/10 px-6 py-3 text-xs font-black uppercase tracking-[0.14em] text-[#25D366] transition-all hover:bg-[#25D366]/20 hover:border-[#25D366]/70 flex items-center justify-center gap-2 mb-3"
                            >
                              <svg
